 ...
 * @package   IMP
 */
class IMP_Ajax_Imple_SwitchKeyDialog extends Horde_Core_Ajax_Imple
{
    /**
    ...
    protected function _attach($init)
    {
        ...
        if ($init) {
            ...
            $page_output->addScriptFile('switchkey.js', 'imp');
        }
        ...
        case 'pgpPersonal':
            $text = _("Choose another personal PGP certificate to decrypt the message: ");
            break;

        case 'pgpSymmetric':
            $text = _("Choose another pgpSymmetric certificate to decrypt the message: ");
            break;

        case 'smimePersonal':
            $text = _("Choose another personal S/MIME Certificate to decrypt: ");
            break;
        }
        ...
        $js = 'ImpSwitchKeyDialog.display(' . Horde::escapeJson($js_params, array('nodelimit' => true)) . ')';
